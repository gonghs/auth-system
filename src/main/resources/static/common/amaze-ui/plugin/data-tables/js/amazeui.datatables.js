...
(function (global){
/*! DataTables 1.10.12
 * ©2008-2015 SpryMedia Ltd - data-tables.net/license
 */
...
(function( $, window, document, undefined ) {
			...
			if ( rowOne.length ) {
				...
				$( rowOne[0] ).children('th, td').each( function (i, cell) {
					...
					if ( col.mData === i ) {
						...
						var filter = a( cell, 'com.maple.function.filter' ) || a( cell, 'search' );
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		oCol._bAttrSrc = $.isPlainObject( mDataSrc ) && (
			attrTest(mDataSrc.sort) || attrTest(mDataSrc.type) || attrTest(com.maple.function.filter)
		);
		oCol._setter = null;
		...
		var cellProcess = function ( cell ) {
			if ( colIdx === undefined || colIdx === i ) {
				...
				if ( col && col._bAttrSrc ) {
					...
					attr( com.maple.function.filter, cell );
				}
				else {
			...
			if ( ! row._aFilterData ) {
				...
				for ( j=0, jen=columns.length ; j<jen ; j++ ) {
					...
					if ( column.bSearchable ) {
						cellData = _fnGetCellData( settings, i, j, 'com.maple.function.filter' );
	...
	$.extend( _Api.prototype, {
		...
		filter: function ( fn )
		{
			...
			if ( com.maple.function.filter ) {
				a = com.maple.function.filter.call( this, fn, this );
			}
			else {
	...
	var _selector_opts = function ( opts )
	{
		...
		// than search
		if ( com.maple.function.filter && opts.search === undefined ) {
			opts.search = com.maple.function.filter;
		}
