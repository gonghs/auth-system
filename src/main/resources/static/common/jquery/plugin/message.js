;(function () {
     ...
     * messageLite v1.0
     * let options={ showClose: true|false, message: '', type:
    ...
    (function (Message) {
        let messageTypeArr = ['error', 'info', 'success', 'warning'];
        for (let index in messageTypeArr) {
            (function () {
                let type = messageTypeArr[index];
                Message[type] = function (option) {
                    let cfg = {};
                    if (ComUtils.isString(option)) {
                        Object.assign(cfg, {message: option})
                    } else {
                        Object.assign(cfg, option)
                    }
                    cfg.type = type;
                    Message(cfg);
                }
